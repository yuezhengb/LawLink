...
import { AccountingFactsWorkspace } from "../_components/accounting-facts-workspace";
...
export default async function InternalFinanceLedgerPage({ searchParams }: { searchParams: Promise<{ period?: string; view?: string }> }) {
  ...
  const canManage = hasCustomPermission(session.user, "finance.adjust");
  const people = canManage ? await prisma.user.findMany({ where: { active: true }, select: { id: true, name: true }, orderBy: { name: "asc" } }) : [];
  ...
  return (
    <div className="space-y-4">
      ...
      <AccountingFactsWorkspace period={period} people={people.map((person) => ({ id: person.id, name: person.name }))} canManage={canManage} />
