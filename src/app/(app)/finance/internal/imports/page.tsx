...
import { canReviewFinanceImport, listFinanceImportReviews } from "@/server/finance/internal-import-review";
...
export default async function InternalFinanceImportsPage() {
  ...
  const [batches, reviewData] = await Promise.all([
    listInternalImportBatches({ actor: session.user }),
    listFinanceImportReviews({ actor: session.user })
  ]);
  return (
    <div className="space-y-4">
      ...
      <ImportWorkspace
        batches={batches}
        reviewRecords={reviewData.records}
        reviewUsers={reviewData.users}
        canImport={canManageFinanceImports(session.user)}
        canReview={canReviewFinanceImport(session.user)}
      />
    </div>
  );
}
