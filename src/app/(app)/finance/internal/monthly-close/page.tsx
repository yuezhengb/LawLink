...
export default async function InternalFinanceMonthlyClosePage({ searchParams }: { searchParams: Promise<{ period?: string }> }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <MonthlyCloseWorkspace data={data} canMaterialize={canManageInternalFinance(session.user, "finance.rules")} canAdjust={canManageInternalFinance(session.user, "finance.adjust")} />
    </div>
  );
}
